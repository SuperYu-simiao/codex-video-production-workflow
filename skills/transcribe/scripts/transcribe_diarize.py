...
DEFAULT_BASE_URL = "https://api.aicodewith.ai/chatgpt/v1"
...
def _ensure_api_key(dry_run: bool) -> None:
    if os.getenv("OPENAI_API_KEY"):
        ...
        return
    if _read_codex_api_key():
        print("OPENAI_API_KEY is set via ~/.codex/auth.json.", file=sys.stderr)
        return
    if dry_run:
...
def _read_codex_api_key() -> Optional[str]:
...
def _read_codex_base_url() -> Optional[str]:
    ...
    provider = providers.get("aicodewith", {})
...
def _create_client():
    ...
    api_key = os.getenv("OPENAI_API_KEY") or _read_codex_api_key()
    base_url = os.getenv("OPENAI_BASE_URL") or _read_codex_base_url() or DEFAULT_BASE_URL
    if not api_key:
        _die("OPENAI_API_KEY is not set. Export it before running.")
